...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "جلب فرع طلب سحب من GitHub والتبديل إليه ثم تشغيل arabcode",
  builder: (yargs) =>
    yargs.positional("number", {
      type: "number",
      describe: "رقم طلب السحب للتبديل إليه",
      demandOption: true,
    }),
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    // index.ts catch handles it identically (exit 1, "خطأ غير متوقع" banner).
