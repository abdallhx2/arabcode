...
const activeSuffix = (isActive: boolean) => (isActive ? dim(" (نشط)") : "")
...
const loginEffect = Effect.fn("login")(function* (url: string) {
  ...
  yield* Prompt.intro("تسجيل الدخول")
  ...
  yield* Match.valueTags(result, {
    ...
    PollDenied: () => s.stop("رُفض التفويض", 1),
...
const logoutEffect = Effect.fn("logout")(function* (email?: string) {
  ...
  if (accounts.length === 0) return yield* println("غير مسجّل الدخول")
  ...
  yield* Prompt.intro("تسجيل الخروج")
  ...
  const selected = yield* Prompt.select({ message: "اختر الحساب لتسجيل الخروج", options: opts })
...
const switchEffect = Effect.fn("switch")(function* () {
  ...
  if (groups.length === 0) return yield* println("غير مسجّل الدخول")
  ...
  if (opts.length === 0) return yield* println("لا توجد منظمات")

  yield* Prompt.intro("تبديل المنظمة")

  const selected = yield* Prompt.select<OrgChoice>({ message: "اختر المنظمة", options: opts })
  if (Option.isNone(selected)) return
...
const orgsEffect = Effect.fn("orgs")(function* () {
  ...
  const groups = yield* service.orgsByAccount()
  if (groups.length === 0) return yield* println("لا توجد حسابات")
  if (!groups.some((group) => group.orgs.length > 0)) return yield* println("لا توجد منظمات")
...
const openEffect = Effect.fn("open")(function* () {
  const service = yield* Account.Service
  const active = yield* service.active()
  if (Option.isNone(active)) return yield* println("لا يوجد حساب نشط")
...
export const LogoutCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs.positional("email", {
      describe: "البريد الإلكتروني للحساب لتسجيل الخروج منه",
      type: "string",
    }),
...
export const ConsoleCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .command({
        ...LoginCommand,
        describe: "تسجيل الدخول إلى وحدة التحكم",
      })
      .command({
        ...LogoutCommand,
        describe: "تسجيل الخروج من وحدة التحكم",
      })
      .command({
        ...SwitchCommand,
        describe: "تبديل المنظمة النشطة",
      })
      .command({
        ...OrgsCommand,
        describe: "عرض المنظمات",
      })
      .command({
        ...OpenCommand,
        describe: "فتح حساب وحدة التحكم النشط",
      })
      .demandCommand(),
  async handler() {},
})
