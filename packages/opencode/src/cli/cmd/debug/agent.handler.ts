import { PermissionV1 } from "@arabcode/core/v1/permission"
import { EOL } from "os"
...
const run = Effect.fn("Cli.debug.agent.body")(function* (
  ...
  if (!agent) {
    process.stderr.write(
      `الوكيل ${agentName} غير موجود، شغّل '${basename(process.execPath)} agent list' للحصول على قائمة الوكلاء` + EOL,
    )
    return yield* fail("", 1)
  }
  ...
  if (toolID) {
    ...
    if (!tool) {
      process.stderr.write(`الأداة ${toolID} غير موجودة للوكيل ${agentName}` + EOL)
      return yield* fail("", 1)
    }
    if (resolvedTools[toolID] === false) {
      process.stderr.write(`الأداة ${toolID} معطّلة للوكيل ${agentName}` + EOL)
      return yield* fail("", 1)
    }
      ...
      Effect.matchCauseEffect({
        ...
        onFailure: (cause) => {
          ...
          if (error instanceof Provider.ModelNotFoundError) {
            return fail(`النموذج غير موجود: ${error.providerID}/${error.modelID}`)
          }
          if (error instanceof Provider.NoModelsError) return fail(`لا توجد نماذج للمزوّد ${error.providerID}`)
          return fail("لم يتم العثور على مزوّدين")
        },
      }),
    ))
...
function parseToolParams(input?: string) {
  ...
  const parsed = iife(() => {
    try {
      ...
      try {
        ...
        throw new Error(
          `فشل تحليل --params. استخدم JSON أو كائن JS حرفي. خطأ JSON: ${jsonError}. خطأ التقييم: ${evalError}.`,
          { cause: evalError },
        )
      }
    }
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("يجب أن تكون معاملات الأداة كائناً.")
  }
...
const createToolContext = Effect.fn("Cli.debug.agent.createToolContext")(function* (
  ...
  const session = yield* sessionSvc.create({ title: `تشغيل أداة للتصحيح (${agent.name})` })
          ...
          Effect.matchCauseEffect({
            ...
            onFailure: (cause) => {
              ...
              if (error instanceof Provider.NoModelsError)
                return fail(`لا توجد نماذج للمزوّد ${error.providerID}`)
              return fail("لم يتم العثور على مزوّدين")
            },
          }),
        )
