...
export const DebugCommand = cmd({
  command: "debug",
  describe: "أدوات التصحيح واستكشاف الأخطاء",
  builder: (yargs) =>
    yargs
...
const WaitCommand = effectCmd({
  command: "wait",
  describe: "الانتظار إلى أجل غير مسمى (للتصحيح)",
...
const InfoCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.debug.info")(function* () {
    ...
    console.log(`إصدار arabcode: ${InstallationVersion}`)
    console.log(`نظام التشغيل: ${os.type()} ${os.release()} ${os.arch()}`)
    console.log(`الطرفية: ${terminal || "غير معروف"}`)
    console.log("الإضافات:")
    if (Flag.OPENCODE_PURE) {
      console.log("الإضافات الخارجية معطّلة (--pure)")
      return
    }
...
const PathsCommand = cmd({
  command: "paths",
  describe: "عرض المسارات العامة (data, config, cache, state)",
  handler() {
