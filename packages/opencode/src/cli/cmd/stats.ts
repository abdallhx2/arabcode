...
export const StatsCommand = effectCmd({
  command: "stats",
  describe: "عرض إحصائيات استهلاك التوكن والتكلفة",
  builder: (yargs) =>
    yargs
      .option("days", {
        describe: "عرض الإحصائيات لآخر N يوم (الافتراضي: كل الوقت)",
        type: "number",
      })
      .option("tools", {
        describe: "عدد الأدوات المعروضة (الافتراضي: الكل)",
        type: "number",
      })
      .option("models", {
        describe: "عرض إحصائيات النماذج (مخفية افتراضياً). مرّر رقماً لعرض أعلى N، وإلا فالكل",
      })
      .option("project", {
        describe: "التصفية حسب المشروع (الافتراضي: كل المشاريع، سلسلة فارغة: المشروع الحالي)",
        type: "string",
      }),
...
export function displayStats(stats: SessionStats, toolLimit?: number, modelLimit?: number) {
  ...
  console.log(renderRow("التكلفة الإجمالية", `$${cost.toFixed(2)}`))
  ...
  console.log(renderRow("قراءة الذاكرة المؤقتة", formatNumber(stats.totalTokens.cache.read)))
  console.log(renderRow("كتابة الذاكرة المؤقتة", formatNumber(stats.totalTokens.cache.write)))
  console.log("└────────────────────────────────────────────────────────┘")
  console.log()
