...
import { OpencodeKeymapProvider } from "@arabcode/tui/keymap"
...
export class RunFooter implements FooterApi {
  ...
  private handlePrompt = (input: RunPrompt): boolean => {
    ...
    if (this.prompts.size === 0) {
      this.setNotice("قائمة انتظار الإدخال غير متوفرة")
      return false
    }
  ...
  private handleCycle = (): void => {
    ...
    if (!result) {
      this.setNotice("لا توجد متغيّرات متاحة")
      return
    }
    ...
    this.patch(patch)
    this.setNotice(result.status ?? "تم تحديث المتغيّر")
  }
  ...
  private handleExit = (): boolean => {
    ...
    this.clearExitTimer()
    this.patch({ exit: 0, status: "جارٍ الخروج" })
    this.close()
    this.options.onExit?.()
    return true
  }
