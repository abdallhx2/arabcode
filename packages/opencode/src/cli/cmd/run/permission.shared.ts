...
import type { PermissionRequest } from "@arabcode/sdk/v2"
...
export function permissionInfo(request: PermissionRequest): PermissionInfo {
  ...
  if (request.permission === "external_directory") {
    ...
    return {
      icon: "←",
      title: `الوصول إلى مجلد خارجي ${toolPath(dir, { home: true })}`,
  ...
  if (request.permission === "doom_loop") {
    return {
      icon: "⟳",
      title: "المتابعة بعد الإخفاقات المتكررة",
      lines: ["يُبقي هذا الجلسة قيد التشغيل رغم الإخفاقات المتكررة."],
    }
  }

  return {
    icon: "⚙",
    title: `استدعاء الأداة ${request.permission}`,
    lines: [`الأداة: ${request.permission}`],
  }
}

export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`سيسمح هذا بـ ${request.permission} حتى إعادة تشغيل arabcode.`]
  }

  return [
    "سيسمح هذا بالأنماط التالية حتى إعادة تشغيل arabcode.",
    ...request.always.map((item) => `- ${item}`),
  ]
}

export function permissionLabel(option: PermissionOption): string {
  if (option === "once") return "السماح مرة"
  if (option === "always") return "السماح دائماً"
  if (option === "reject") return "رفض"
  if (option === "confirm") return "تأكيد"
  return "إلغاء"
}
