...
import type { ToolPart } from "@arabcode/sdk/v2"
...
function fail(ctx: ToolFrame): string {
  ...
  if (error) {
    return `✖ فشلت الأداة ${ctx.name}: ${error}`
  }

  return `✖ فشلت الأداة ${ctx.name}`
}
...
function fallbackFinal(ctx: ToolFrame): string {
  ...
  if (!time) {
    return `اكتمل ${ctx.name}`
  }

  return `اكتمل ${ctx.name} · ${time}`
}
...
function fallbackInline(ctx: ToolFrame): ToolInline {
  const title = text(ctx.state.title) || (Object.keys(ctx.input).length > 0 ? JSON.stringify(ctx.input) : "غير معروف")
...
function runGlob(p: ToolProps<typeof GlobTool>): ToolInline {
  ...
  const suffix = root ? `في ${toolPath(root)}` : ""
  const matches = p.metadata.count
  const description = matches === undefined ? suffix : `${suffix}${suffix ? " · " : ""}${count(matches, "تطابق")}`
  return {
    icon: "✱",
    title,
...
function runList(p: ToolProps): ToolInline {
  ...
  return {
    icon: "→",
    title: dir ? `سرد ${toolPath(dir)}` : "سرد",
  }
}
...
function runWrite(p: ToolProps<typeof WriteTool>): ToolInline {
  return {
    icon: "←",
    title: `كتابة ${toolPath(p.input.filePath)}`,
    mode: "block",
...
function runEdit(p: ToolProps<typeof EditTool>): ToolInline {
  return {
    icon: "←",
    title: `تحرير ${toolPath(p.input.filePath)}`,
    mode: "block",
...
function runTask(p: ToolProps<typeof TaskTool>): ToolInline {
  const kind = Locale.titlecase(p.input.subagent_type || "غير معروف")
  const desc = p.input.description
  const icon = p.frame.status === "error" ? "✗" : p.frame.status === "running" ? "•" : "✓"
  return {
    icon,
    title: desc || `${kind} مهمة`,
    description: desc ? `${kind} وكيل` : undefined,
  }
}
...
function runPatch(p: ToolProps<typeof ApplyPatchTool>): ToolInline {
  ...
  return {
    icon: "%",
    title: `ترقيع ${files} ${files === 1 ? "ملف" : "ملفات"}`,
  }
}
...
function runQuestion(p: ToolProps<typeof QuestionTool>): ToolInline {
  ...
  return {
    icon: "→",
    title: `تم طرح ${total} ${total === 1 ? "سؤال" : "أسئلة"}`,
  }
}
...
function runInvalid(p: ToolProps<typeof InvalidTool>): ToolInline {
  return {
    icon: "✗",
    title: text(p.frame.state.title) || "أداة غير صالحة",
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
...
function runBatch(p: ToolProps): ToolInline {
  ...
  return {
    icon: "#",
    title: text(p.frame.state.title) || (calls > 0 ? `دفعة ${calls} ${calls === 1 ? "أداة" : "أدوات"}` : "دفعة"),
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
...
function runPlanExit(p: ToolProps<typeof PlanExitTool>): ToolInline {
  return {
    icon: "→",
    title: text(p.frame.state.title) || "جارٍ التبديل إلى وكيل build",
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  }
}
...
function patchTitle(file: PatchFile): string {
  ...
  if (file.type === "add") {
    return `# تم إنشاء ${rel || toolPath(from)}`
  }
  if (file.type === "delete") {
    return `# تم حذف ${rel || toolPath(from)}`
  }
  if (file.type === "move") {
    return `# تم نقل ${toolPath(from)} -> ${rel || toolPath(file.movePath)}`
  }

  return `# تم ترقيع ${rel || toolPath(from)}`
}
...
function snapQuestion(p: ToolProps<typeof QuestionTool>): ToolSnapshot {
  ...
  const items = list<{ question?: string }>(p.frame.input.questions).map((item, i) => {
    ...
    return {
      question: item.question || `سؤال ${i + 1}`,
      answer: answer.length > 0 ? answer.join(", ") : "(بلا إجابة)",
    }
  })
...
function scrollBashStart(p: ToolProps<typeof BashTool>): string {
  ...
  if (!cmd) {
    return dir ? `# جارٍ التشغيل في ${dir}` : ""
  }

  return `# جارٍ التشغيل في ${dir}\n$ ${cmd}`
}
...
function scrollBashFinal(p: ToolProps<typeof BashTool>): string {
  ...
  if (code === undefined) {
    ...
    return `اكتمل bash · ${time}`
  }

  return `اكتمل bash (رمز الخروج ${code})${time ? ` · ${time}` : ""}`
}
...
function scrollPatchFinal(p: ToolProps<typeof ApplyPatchTool>): string {
  ...
  if (shown.length > 6) {
    rows.push(`... و${shown.length - 6} أخرى`)
  }
...
function scrollTodoFinal(p: ToolProps<typeof TodoWriteTool>): string {
  ...
  const tail = [`${items.length} إجمالاً`]
  if (doneN > 0) {
    tail.push(`${doneN} منجزة`)
  }
  if (runN > 0) {
    tail.push(`${runN} نشطة`)
  }
  if (left > 0) {
    tail.push(`${left} قيد الانتظار`)
  }
...
function scrollQuestionFinal(p: ToolProps<typeof QuestionTool>): string {
  ...
  for (const [i, item] of q.slice(0, 4).entries()) {
    ...
    const reply = a[i] ?? []
    rows.push(`? ${prompt || `سؤال ${i + 1}`}`)
    rows.push(`  ${reply.length > 0 ? reply.join(", ") : "(بلا إجابة)"}`)
  }

  if (q.length > 4) {
    rows.push(`... و${q.length - 4} أخرى`)
  }
...
function permEdit(p: ToolPermissionProps<typeof EditTool>): ToolPermissionInfo {
  ...
  return {
    icon: "→",
    title: `تحرير ${toolPath(file, { home: true })}`,
    lines: [],
...
function permRead(p: ToolPermissionProps<typeof ReadTool>): ToolPermissionInfo {
  ...
  return {
    icon: "→",
    title: `قراءة ${toolPath(file, { home: true })}`,
    lines: file ? [`المسار: ${toolPath(file, { home: true })}`] : [],
  }
}
...
function permList(p: ToolPermissionProps): ToolPermissionInfo {
  ...
  return {
    icon: "→",
    title: `سرد ${toolPath(dir, { home: true })}`,
    lines: dir ? [`المسار: ${toolPath(dir, { home: true })}`] : [],
  }
}
...
function permTask(p: ToolPermissionProps<typeof TaskTool>): ToolPermissionInfo {
  ...
  return {
    icon: "#",
    title: `${Locale.titlecase(type)} مهمة`,
    lines: desc ? [`◉ ${desc}`] : [],
  }
}
...
function permLsp(p: ToolPermissionProps<typeof LspTool>): ToolPermissionInfo {
  ...
  return {
    ...
    lines: [
      ...(p.input.operation ? [`العملية: ${p.input.operation}`] : []),
      ...(file ? [`المسار: ${toolPath(file, { home: true })}`] : []),
      ...(pos ? [`الموضع: ${pos}`] : []),
    ],
  }
}
