...
import type { QuestionInfo, QuestionRequest } from "@arabcode/sdk/v2"
...
export function questionHint(request: QuestionRequest, state: QuestionBodyState): string {
  if (state.submitting) {
    return "جارٍ انتظار حدث السؤال..."
  }

  if (questionConfirm(request, state)) {
    return "enter إرسال   esc تجاهل"
  }

  if (state.editing) {
    return "enter حفظ   esc إلغاء"
  }

  const info = questionInfo(request, state)
  if (questionSingle(request)) {
    return `↑↓ اختيار   enter ${info?.multiple ? "تبديل" : "إرسال"}   esc تجاهل`
  }

  return `⇆ tab   ↑↓ اختيار   enter ${info?.multiple ? "تبديل" : "تأكيد"}   esc تجاهل`
}
