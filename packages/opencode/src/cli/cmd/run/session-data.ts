...
import type { Event, Part, PermissionRequest, QuestionRequest, ToolPart } from "@arabcode/sdk/v2"
...
function toolStatus(part: ToolPart): string {
  ...
  if (typeof desc === "string" && desc.trim()) {
    return `جارٍ تشغيل ${desc.trim()}`
  }

  const type = state.input?.subagent_type
  if (typeof type === "string" && type.trim()) {
    return `جارٍ تشغيل ${type.trim()}`
  }

  return "جارٍ تشغيل المهمة"
}
...
function flushPart(data: SessionData, commits: SessionCommit[], partID: string, interrupted = false) {
  ...
  if (sent === 0) {
    ...
    if (kind === "reasoning" && chunk) {
      chunk = `تفكير: ${chunk.replace(/\[REDACTED\]/g, "")}`
    }
...
export function reduceSessionData(input: SessionDataInput): SessionDataOutput {
  ...
  if (event.type === "session.next.shell.started") {
    ...
    if (data.ids.has(partID) || data.tools.has(partID)) {
      return out(data, commits, patch({ status: "جارٍ تشغيل shell" }))
    }
  ...
  if (event.type === "message.part.updated") {
    ...
    if (part.type === "tool") {
      ...
      if (part.state.status === "error") {
        ...
        data.ids.add(part.id)
        const text =
          typeof part.state.error === "string" && part.state.error.trim() ? part.state.error : "خطأ غير معروف"
