...
function categoryRank(category: string) {
  if (category === "أوامر المشروع") {
    return 0
  }

  if (category === "أوامر MCP") {
    return 1
  }
...
export function RunCommandMenuBody(props: {
  ...
  const entries = createMemo<CommandEntry[]>(() => {
        ...
        ? [
            {
              ...
              category: "الجلسة",
              display: "عرض الوكلاء الفرعيين",
              footer:
                activeSubagentCount() > 0 ? `${activeSubagentCount()} نشط` : `${props.subagents().length} الأخيرة`,
              keywords: props
                .subagents()
        ...
        ? [
            {
              ...
              category: "الوكيل",
              display: "إدارة الطلبات المنتظرة",
              footer: `${props.queued().length} في الانتظار`,
              keywords: props
                .queued()
    ...
    const commands = (props.commands() ?? [])
      ...
      .map(
        (item) =>
          ({
            action: "slash",
            category: item.source === "mcp" ? "أوامر MCP" : "أوامر المشروع",
            name: item.name,
            display: item.name,
            footer: `/${item.name}`,
            keywords:
    ...
    return [
      ...
      ...agent,
      ...commands,
      { action: "exit", category: "النظام", display: "خروج", footer: "/exit", keywords: "/exit exit" },
    ]
  })
...
export function RunQueuedPromptSelectBody(props: {
  ...
  const entries = createMemo<QueuedEntry[]>(() =>
    props.prompts().map((prompt) => ({
      ...
      footer: "في الانتظار · ctrl+e تحرير · ctrl+d إزالة",
  ...
  return (
    ...
    >
      <RunFooterMenu
        ...
        rows={() => PANEL_LIST_ROWS}
        limit={PANEL_LIST_ROWS}
        empty={props.commands() ? "لا توجد مهارات" : "جارٍ تحميل المهارات"}
        border={false}
        paddingLeft={PANEL_PAD}
        paddingRight={PANEL_PAD}
        grouped={false}
  ...
  const entries = createMemo<VariantEntry[]>(() => [
    {
      category: "",
      display: "افتراضي",
      description: props.current() === undefined ? "الحالي" : undefined,
      keywords: "default",
      variant: undefined,
    ...
    ...props.variants().map((variant) => ({
      category: "",
      display: variant,
      description: props.current() === variant ? "الحالي" : undefined,
      keywords: variant,
      variant,
  ...
  return (
    ...
    >
      <RunFooterMenu
        ...
        rows={() => PANEL_LIST_ROWS}
        limit={PANEL_LIST_ROWS}
        empty={props.providers() ? "لا توجد نتائج" : "جارٍ تحميل النماذج"}
        border={false}
        paddingLeft={PANEL_PAD}
        paddingRight={PANEL_PAD}
        grouped={!query().trim()}
