...
import { createOpencodeClient } from "@arabcode/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput, deps: RunRuntimeDeps = {}): Promise<void> {
  ...
  const shell = await (deps.createRuntimeLifecycle ?? createRuntimeLifecycle)({
    ...
    onCycleVariant: () => {
      ...
      return {
        status: state.activeVariant ? `المتغيّر ${state.activeVariant}` : "المتغيّر الافتراضي",
    ...
    onVariantSelect: async (variant) => {
      ...
      if (variant && !state.variants.includes(variant)) {
        return {
          status: `المتغيّر ${variant} غير متاح`,
        }
      }
  ...
  if (Flag.OPENCODE_SHOW_TTFD) {
    footer.append({
      kind: "system",
      text: `بدء التشغيل ${Math.max(0, Math.round(performance.now() - start))} مللي ثانية`,
      phase: "final",
      source: "system",
    })
  }
      ...
      onNewSession: createSession
        ? async () => {
            try {
              ...
              footer.append({
                kind: "system",
                text: `جلسة جديدة ${state.sessionID}`,
                phase: "final",
                source: "system",
              })
              await state.demo?.start()
