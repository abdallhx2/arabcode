...
export const SPLASH_TITLE_FALLBACK = "جلسة بلا عنوان"
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    push(lines, body_left, top, "arabcode", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    const top = 1
    const body_left = (mark[0]?.length ?? 0) + 2
    const session = "الجلسة  "
    const label = "للمتابعة "
    ...
    push(
      ...
      top + 1,
      `arabcode --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
