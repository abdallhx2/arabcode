...
import { normalizePromptContent } from "@arabcode/tui/editor"
...
import { OPENCODE_BASE_MODE, useBindings } from "@arabcode/tui/keymap"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const placeholder = createMemo(() => {
    if (shell()) {
      return new StyledText([fg(input.theme().muted)('شغّل أمراً... "git status"')])
    }
    ...
    return new StyledText([fg(input.theme().muted)('اسأل أي شيء... "Fix a TODO in the codebase"')])
  })
    ...
    const builtins = [
      {
        ...
        name: "editor",
        display: "/editor",
        description: "الكتابة في محرّرك الخارجي",
      } satisfies SlashOption,
      { kind: "slash", name: "new", display: "/new", description: "بدء جلسة جديدة" } satisfies SlashOption,
      { kind: "slash", name: "exit", display: "/exit", description: "إغلاق arabcode" } satisfies SlashOption,
    ]
        ...
        ? [
            {
              ...
              name: "skills",
              display: "/skills",
              description: "تصفّح المهارات المتاحة",
            } satisfies SlashOption,
          ]
        : []),
      ...(input.commands() ?? [])
  ...
  const openEditor = async (inputValue?: { value?: string }) => {
    ...
    try {
      ...
      restore(current)
      input.onStatus("فشل فتح المحرّر")
    }
  }
  ...
  useBindings(() => ({
    ...
    commands: [
      {
        name: "prompt.clear",
        title: "مسح مربع الكتابة أو الخروج",
    ...
    commands: [
      {
        ...
        title: "عنصر الإكمال التلقائي السابق",
        category: "الإكمال التلقائي",
        run: () => menu.move(-1),
      },
      {
        name: "prompt.autocomplete.next",
        title: "عنصر الإكمال التلقائي التالي",
        category: "الإكمال التلقائي",
        run: () => menu.move(1),
      },
      {
        name: "prompt.autocomplete.hide",
        title: "إخفاء الإكمال التلقائي",
        category: "الإكمال التلقائي",
        run: cancelAutocomplete,
      },
      {
        name: "prompt.autocomplete.select",
        title: "اختيار عنصر الإكمال التلقائي",
        category: "الإكمال التلقائي",
        run() {
      ...
      },
      {
        ...
        title: "إتمام عنصر الإكمال التلقائي",
        category: "الإكمال التلقائي",
        run() {
          if (mode() === "slash" && options().length === 0) {
            hide()
  ...
  const submitPrompt = (next: RunPrompt) => {
    ...
    if (!next.text.trim()) {
      input.onStatus(input.state().phase === "running" ? "بانتظار الرد الحالي" : "تم تجاهل الطلب الفارغ")
      return
    }
    ...
    if (parsed?.type === "pending") {
      input.onStatus("جارٍ تحميل الأوامر")
      return
    }
