...
import type { Event, GlobalEvent, OpencodeClient } from "@arabcode/sdk/v2"
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const resolveShellAgent = Effect.fn("RunStreamTransport.resolveShellAgent")(function* (
          ...
          return yield* Effect.fail(new Error("لا يوجد وكيل رئيسي متاح لوضع shell"))
        })
        ...
        const replayOnResize: (next: SessionResizeReplayInput) => Effect.Effect<boolean> = Effect.fn(
          ...
          if (Exit.isFailure(reset)) {
            ...
            input.footer.append({
              kind: "error",
              text: "فشل إعادة العرض بعد تغيير الحجم؛ عُطّل لهذه الجلسة",
              phase: "start",
              source: "system",
            })
            yield* finish()
            ...
            Stream.runForEach(
              Effect.fn("RunStreamTransport.event")(function* (item: unknown) {
                ...
                if (isMatchingDisposeEvent(item, input.directory)) {
                  yield* fail(new Error("تم إنهاء نسخة الخادم"))
                  yield* closeScope()
                  return
                }
            ...
            Effect.ensuring(
              Effect.gen(function* () {
                if (!abort.signal.aborted && !state.fault) {
                  yield* fail(new Error("أُغلق تدفق الأحداث العام"))
                }
                closeStream()
              }),
            ),
        ...
        const runPromptTurn = Effect.fn("RunStreamTransport.runPromptTurn")(function* (next: SessionTurnInput) {
          ...
          if (state.wait) {
            yield* Effect.fail(new Error("هناك طلب قيد التشغيل بالفعل"))
            return
          }
