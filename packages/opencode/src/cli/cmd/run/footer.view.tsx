...
import { registerOpencodeSpinner } from "@arabcode/tui/component/register-spinner"
import { createColors, createFrames } from "@arabcode/tui/ui/spinner"
import {
...
export function RunFooterView(props: RunFooterViewProps) {
  ...
  const statusText = createMemo(() => {
    if (exiting()) {
      return `اضغط ${clearShortcut() || "ctrl+c"} مرة أخرى للخروج`
    }

    if (busy()) {
      return armed() ? "مرة أخرى للمقاطعة" : "مقاطعة"
    }
  ...
  const contextHints = createMemo(() => {
    ...
    if (foregroundSubagents() && backgroundShortcut()) {
      items.push({ kind: "background", key: backgroundShortcut(), label: "الخلفية" })
    }
    if (queuedPrompts().length > 0 && queuedShortcut()) {
      items.push({ kind: "queued", key: queuedShortcut(), label: `${queue()} في الانتظار` })
    }
    if (activeTabs().length > 0 && subagentShortcut()) {
      items.push({ kind: "subagents", key: subagentShortcut(), label: "وكلاء فرعيون" })
    }
  ...
  const commandHint = createMemo(() => {
    ...
    if (shell()) {
      return { key: "esc", label: "عادي" }
    }

    if (command()) {
      return { key: command(), label: "أمر" }
    }
  })
    ...
    commands: [
      {
        ...
        title: "تشغيل الوكلاء الفرعيين في الخلفية",
        category: "الجلسة",
