...
import type { Hooks } from "@arabcode/plugin"
  ...
  if (method.type === "oauth") {
    ...
    if (authorize.method === "auto") {
      ...
      if (result.type === "success") {
        ...
        yield* spinner.stop("تم تسجيل الدخول بنجاح")
      }
    }
    ...
    if (authorize.method === "code") {
      ...
      if (result.type === "failed") {
        yield* Prompt.log.error("فشل التفويض")
      }
      if (result.type === "success") {
        const saveProvider = result.provider ?? provider
        if ("refresh" in result) {
        ...
        yield* Prompt.log.success("تم تسجيل الدخول بنجاح")
      }
    }
...
export const ProvidersCommand = cmd({
  command: "providers",
  aliases: ["auth"],
  describe: "إدارة مزوّدي الذكاء الاصطناعي وبيانات الاعتماد",
  builder: (yargs) =>
...
export const ProvidersListCommand = effectCmd({
  command: "list",
  aliases: ["ls"],
  describe: "عرض المزوّدين وبيانات الاعتماد",
...
export const ProvidersLoginCommand = effectCmd({
  command: "login [url]",
  describe: "تسجيل الدخول إلى مزوّد",
  ...
  builder: (yargs: Argv) =>
    yargs
      .positional("url", {
        describe: "مزوّد مصادقة arabcode",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
        describe: "معرّف المزوّد أو اسمه لتسجيل الدخول (يتخطى اختيار المزوّد)",
        type: "string",
      })
      .option("method", {
        alias: ["m"],
        describe: "تسمية طريقة تسجيل الدخول (تتخطى اختيار الطريقة)",
        type: "string",
      }),
...
export const ProvidersLogoutCommand = effectCmd({
  ...
  describe: "تسجيل الخروج من مزوّد مُعدّ",
  builder: (yargs) =>
    yargs.positional("provider", {
      describe: "معرّف المزوّد أو اسمه لتسجيل الخروج منه",
      type: "string",
    }),
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    if (credentials.length === 0) {
      yield* Prompt.log.error("لا توجد بيانات اعتماد")
      return
    }
    const database = yield* modelsDev.get()
    ...
    yield* Prompt.outro("تم تسجيل الخروج بنجاح")
  }),
})
