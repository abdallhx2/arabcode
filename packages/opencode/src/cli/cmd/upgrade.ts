...
export const UpgradeCommand = {
  ...
  handler: async (args: { target?: string; method?: string }) => {
    ...
    if (method === "unknown") {
      prompts.log.error(`arabcode is installed to ${process.execPath} and may be managed by a package manager`)
    ...
    if (InstallationVersion === target) {
      prompts.log.warn(`arabcode upgrade skipped: ${target} is already installed`)
      prompts.outro("Done")
      return
    }
