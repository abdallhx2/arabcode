import type { PermissionV1 } from "@arabcode/core/v1/permission"
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@arabcode/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "تشغيل arabcode برسالة",
  ...
  builder: (yargs: Argv) =>
    yargs
      .positional("message", {
        describe: "الرسالة المراد إرسالها",
        type: "string",
        array: true,
        default: [],
      })
      .option("command", {
        describe: "الأمر المراد تشغيله، استخدم message للوسائط",
        type: "string",
      })
      .option("continue", {
        alias: ["c"],
        describe: "متابعة آخر جلسة",
        type: "boolean",
      })
      .option("session", {
        alias: ["s"],
        describe: "معرّف الجلسة للمتابعة",
        type: "string",
      })
      .option("fork", {
        describe: "تفريع الجلسة قبل المتابعة (يتطلب --continue أو --session)",
        type: "boolean",
      })
      .option("share", {
        type: "boolean",
        describe: "مشاركة الجلسة",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
        describe: "النموذج المستخدَم بصيغة provider/model",
      })
      .option("agent", {
        type: "string",
        describe: "الوكيل المستخدَم",
      })
      .option("format", {
        type: "string",
        choices: ["default", "json"],
        default: "default",
        describe: "الصيغة: default (منسّقة) أو json (أحداث JSON خام)",
      })
      .option("file", {
        alias: ["f"],
        type: "string",
        array: true,
        describe: "ملف/ملفات لإرفاقها بالرسالة",
      })
      .option("title", {
        type: "string",
        describe: "عنوان الجلسة (يستخدم طلباً مختصراً إذا لم تُحدَّد قيمة)",
      })
      .option("attach", {
        type: "string",
        describe: "الاتصال بخادم arabcode قيد التشغيل (مثال: http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "كلمة مرور المصادقة الأساسية (الافتراضي OPENCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "اسم مستخدم المصادقة الأساسية (الافتراضي OPENCODE_SERVER_USERNAME أو 'opencode')",
      })
      .option("dir", {
        type: "string",
        describe: "المجلد المراد التشغيل فيه، أو مساره على الخادم البعيد عند الاتصال",
      })
      .option("port", {
        type: "number",
        describe: "منفذ الخادم المحلي (منفذ عشوائي افتراضياً إن لم تُحدَّد قيمة)",
      })
      .option("variant", {
        type: "string",
        describe: "متغيّر النموذج (جهد استدلال خاص بالمزوّد، مثل high أو max أو minimal)",
      })
      .option("thinking", {
        type: "boolean",
        describe: "عرض كتل التفكير",
      })
      .option("mini", {
        type: "boolean",
        hidden: true,
      ...
      .option("replay", {
        ...
        default: true,
        hidden: true,
        describe: "إعادة عرض سجل الجلسة التفاعلي عند الاستئناف وبعد تغيير الحجم (استخدم --no-replay للتعطيل)",
      })
      .option("replay-limit", {
        type: "number",
        hidden: true,
        describe: "تحديد إعادة العرض التفاعلي بأحدث N رسالة",
      })
      .option("interactive", {
        alias: ["i"],
        type: "boolean",
        describe: "التشغيل في الوضع التفاعلي المباشر بفوتر منقسم",
        default: false,
      })
      .option("auto", {
        type: "boolean",
        describe: "الموافقة تلقائياً على الأذونات غير المرفوضة صراحةً (خطر!)",
        default: false,
      })
      .option("yolo", {
        type: "boolean",
