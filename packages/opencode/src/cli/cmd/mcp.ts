import { cmd } from "./cmd"
import { ConfigV1 } from "@arabcode/core/v1/config/config"
...
import { ConfigMCPV1 } from "@arabcode/core/v1/config/mcp"
...
export const McpCommand = cmd({
  command: "mcp",
  describe: "إدارة خوادم MCP (بروتوكول سياق النموذج)",
  builder: (yargs) =>
    yargs
...
export const McpListCommand = effectCmd({
  command: "list",
  aliases: ["ls"],
  describe: "عرض خوادم MCP وحالتها",
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      prompts.log.warn("لا توجد خوادم MCP مُعدّة")
...
export const McpAuthCommand = effectCmd({
  command: "auth [name]",
  describe: "المصادقة مع خادم MCP يدعم OAuth",
  builder: (yargs) =>
    yargs
      .positional("name", {
        describe: "اسم خادم MCP",
        type: "string",
      })
  ...
  "mcp": {
    ...
    if (!serverName) {
      ...
      const selected = yield* Effect.promise(() =>
        prompts.select({
          message: "اختر خادم MCP للمصادقة",
          options,
        }),
      )
    ...
    spinner.start("جارٍ بدء تدفق OAuth...")

    yield* MCP.Service.use((mcp) =>
      mcp.authenticate(serverName, (url) => {
        spinner.stop("Authorize in your browser:")
        prompts.log.info(url)
        spinner.start("بانتظار التفويض...")
      }),
    ).pipe(
      Effect.tap((status) =>
        Effect.sync(() => {
          if (status.status === "connected") {
            spinner.stop("نجحت المصادقة!")
          } else if (status.status === "needs_client_registration") {
            spinner.stop("فشلت المصادقة", 1)
...
export const McpAuthListCommand = effectCmd({
  command: "list",
  aliases: ["ls"],
  describe: "عرض خوادم MCP الداعمة لـ OAuth وحالة مصادقتها",
...
export const McpLogoutCommand = effectCmd({
  command: "logout [name]",
  describe: "إزالة بيانات اعتماد OAuth لخادم MCP",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.mcp.logout")(function* (args) {
    ...
    if (!serverName) {
      const selected = yield* Effect.promise(() =>
        prompts.select({
          message: "اختر خادم MCP لتسجيل الخروج",
...
export const McpAddCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      ...
      .option("url", {
        describe: "رابط خادم MCP بعيد",
        type: "string",
      })
      .option("env", {
        describe: "متغيّر بيئة لخادم MCP محلي (KEY=VALUE)",
        type: "string",
        array: true,
      })
      .option("header", {
        describe: "ترويسة HTTP لخادم MCP بعيد (KEY=VALUE)",
        type: "string",
        array: true,
      }),
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const name = await prompts.text({
        message: "أدخل اسم خادم MCP",
        validate: (x) => (x && x.length > 0 ? undefined : "Required"),
      })
      if (prompts.isCancel(name)) throw new UI.CancelledError()

      const type = await prompts.select({
        message: "اختر نوع خادم MCP",
        options: [
          {
            label: "Local",
            value: "local",
      ...
      if (type === "local") {
        const command = await prompts.text({
          message: "أدخل الأمر المراد تشغيله",
        ...
        prompts.outro("تمت إضافة خادم MCP بنجاح")
        return
      }

      if (type === "remote") {
        const url = await prompts.text({
          message: "أدخل رابط خادم MCP",
          placeholder: "e.g., https://example.com/mcp",
          validate: (x) => {
            if (!x) return "Required"
            if (x.length === 0) return "Required"
            const isValid = URL.canParse(x)
            return isValid ? undefined : "رابط غير صالح"
          },
        })
...
export const McpDebugCommand = effectCmd({
  command: "debug <name>",
  describe: "تصحيح اتصال OAuth لخادم MCP",
  builder: (yargs) =>
    yargs.positional("name", {
      describe: "اسم خادم MCP",
      type: "string",
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const spinner = prompts.spinner()
      spinner.start("جارٍ اختبار الاتصال...")
