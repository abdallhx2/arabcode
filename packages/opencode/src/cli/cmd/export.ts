...
export const ExportCommand = effectCmd({
  ...
  describe: "تصدير بيانات الجلسة بصيغة JSON",
  builder: (yargs) =>
    yargs
      .positional("sessionID", {
        describe: "معرّف الجلسة للتصدير",
        type: "string",
      })
      .option("sanitize", {
        describe: "حجب بيانات النص والملفات الحساسة",
        type: "boolean",
      }),
...
const run = Effect.fn("Cli.export.body")(function* (args: { sessionID?: string; sanitize?: boolean }) {
  ...
  if (!sessionID) {
    UI.empty()
    prompts.intro("تصدير الجلسة", { output: process.stderr })

    const sessions = yield* svc.list()

    if (sessions.length === 0) {
      prompts.log.error("لا توجد جلسات", { output: process.stderr })
    ...
    prompts.outro("جارٍ تصدير الجلسة...", { output: process.stderr })
  }
