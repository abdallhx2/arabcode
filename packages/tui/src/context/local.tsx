...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    function createAgent() {
      ...
      return {
        ...
        color(name: string) {
          ...
          // ألوان الوضعين الأساسيين ثابتة بالاسم لا بالترتيب: البناء ذهبيّ، التخطيط أزرق.
          if (name === "build") return theme.accent
          if (name === "plan") return theme.secondary
    ...
    function createModel() {
      ...
      return {
        ...
        parsed: createMemo(() => {
          ...
          if (!value) {
            return {
              provider: "اتصل بمزوّد",
              model: "لم يتم اختيار مزوّد",
              reasoning: false,
            }
          }
        ...
        cycleFavorite(direction: 1 | -1) {
          ...
          if (!favorites.length) {
            toast.show({
              variant: "info",
              message: "أضف نموذجاً مفضلاً لاستخدام هذا الاختصار",
              duration: 3000,
            })
            return
          }
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            if (!isModelValid(model)) {
              toast.show({
                message: `النموذج ${model.providerID}/${model.modelID} غير صالح`,
                variant: "warning",
                duration: 3000,
              })
              return
    ...
    createEffect(() => {
      ...
      toast.show({
        variant: "warning",
        message: `النموذج ${value.model.providerID}/${value.model.modelID} المضبوط للوكيل ${value.name} غير صالح`,
        duration: 3000,
      })
    })
