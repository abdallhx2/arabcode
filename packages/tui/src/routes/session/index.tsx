...
export function Session() {
  ...
  function enterChild(sessionID: string) {
    ...
    if (status?.type === "retry") void DialogAlert.show(dialog, "إعادة محاولة الخطأ", status.message)
  }
  ...
  const sessionCommandList = createMemo(() => [
    {
      title: session()?.share?.url ? "نسخ رابط المشاركة" : "مشاركة الجلسة",
      value: "session.share",
      ...
      run: async () => {
        const copy = (url: string) =>
          clipboard
            .write?.(url)
            .then(() => toast.show({ message: "تم نسخ رابط المشاركة إلى الحافظة!", variant: "success" }))
            .catch(() => toast.show({ message: "فشل نسخ الرابط إلى الحافظة", variant: "error" }))
        ...
        if (!kv.get("share_consent", false)) {
          const ok = await DialogConfirm.show(dialog, "مشاركة الجلسة", "هل أنت متأكد أنك تريد مشاركتها؟")
          if (ok !== true) return
        ...
        await sdk.client.session
          ...
          .catch((error) => {
            toast.show({
              message: error instanceof Error ? error.message : "فشل مشاركة الجلسة",
              variant: "error",
            })
          })
        dialog.clear()
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!selectedModel) {
          toast.show({
            variant: "warning",
            message: "اربط مزوّداً لتلخيص هذه الجلسة",
            duration: 3000,
          })
          return
        }
    ...
    },
    {
      ...
      run: async () => {
        await sdk.client.session
          ...
          .then(() => toast.show({ message: "تم إلغاء مشاركة الجلسة بنجاح", variant: "success" }))
          .catch((error) => {
            toast.show({
              message: error instanceof Error ? error.message : "فشل إلغاء مشاركة الجلسة",
              variant: "error",
            })
          })
        dialog.clear()
    ...
    {
      title: "التراجع عن الرسالة السابقة",
      value: "session.undo",
      category: "الجلسة",
      slash: {
        name: "undo",
    ...
    {
      title: conceal() ? "تعطيل إخفاء الكود" : "تفعيل إخفاء الكود",
    ...
    {
      title: showTimestamps() ? "إخفاء الطوابع الزمنية" : "إظهار الطوابع الزمنية",
    ...
    },
    {
      title: (() => {
        ...
        if (next === "hide") return "طيّ التفكير"
        return "بسط التفكير"
      })(),
    ...
    {
      title: showDetails() ? "إخفاء تفاصيل الأداة" : "إظهار تفاصيل الأداة",
    ...
    {
      title: showGenericToolOutput() ? "إخفاء مخرجات الأدوات العامة" : "إظهار مخرجات الأدوات العامة",
    ...
    {
      title: "الانتقال إلى آخر رسالة للمستخدم",
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!lastAssistantMessage) {
          toast.show({ message: "لا توجد رسائل للمساعد", variant: "error" })
          dialog.clear()
          return
        }
        ...
        if (textParts.length === 0) {
          toast.show({ message: "لا توجد أجزاء نصية في آخر رسالة للمساعد", variant: "error" })
          dialog.clear()
          return
        }
        ...
        if (!text) {
          toast.show({
            message: "لا يوجد محتوى نصي في آخر رسالة للمساعد",
            variant: "error",
          })
          dialog.clear()
          return
        ...
        clipboard
          .write?.(text)
          .then(() => toast.show({ message: "تم نسخ الرسالة إلى الحافظة!", variant: "success" }))
          .catch(() => toast.show({ message: "فشل النسخ إلى الحافظة", variant: "error" }))
        dialog.clear()
      },
    },
    {
      ...
      run: async () => {
        try {
          ...
          toast.show({ message: "تم نسخ نص الجلسة إلى الحافظة!", variant: "success" })
        } catch {
          toast.show({ message: "فشل نسخ نص الجلسة", variant: "error" })
        }
        dialog.clear()
      },
    },
    {
      title: "تصدير نص الجلسة",
      ...
      run: async () => {
        try {
          ...
          toast.show({ message: "فشل تصدير الجلسة", variant: "error" })
        }
        dialog.clear()
      },
    },
    {
      title: "الوكلاء الفرعيون في الخلفية",
    ...
    {
      title: "الانتقال إلى الجلسة الفرعية",
    ...
    {
      title: "الانتقال إلى الجلسة الأم",
  ...
  return (
    <LocationProvider location={location()}>
      ...
      >
        <box flexDirection="row-reverse" flexGrow={1} minHeight={0}>
                        ...
                        {(function () {
                          ...
                          const handleUnrevert = async () => {
                            const confirmed = await DialogConfirm.show(
                              dialog,
                              "تأكيد الإعادة",
                              "هل أنت متأكد أنك تريد استعادة الرسائل المتراجَع عنها؟",
                            )
                            if (confirmed) {
...
function GenericTool(props: ToolProps) {
  ...
  return (
    <Show
      ...
      fallback={
        <InlineTool icon="⚙" pending="جارٍ كتابة الأمر..." complete={true} part={props.part}>
      ...
      >
        <box gap={1}>
          ...
          <Show when={collapsed().overflow}>
            <text fg={theme.textMuted}>{expanded() ? "انقر للطي" : "انقر للبسط"}</text>
          </Show>
        </box>
      </BlockTool>
    </Show>
...
function Shell(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="$" pending="جارٍ كتابة الأمر..." complete={stringValue(props.input.command)} part={props.part}>
...
function Glob(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="جارٍ البحث عن الملفات..." complete={stringValue(props.input.pattern)} part={props.part}>
...
function Grep(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="جارٍ البحث في المحتوى..." complete={stringValue(props.input.pattern)} part={props.part}>
...
function WebFetch(props: ToolProps) {
  return (
    <InlineTool icon="%" pending="جارٍ الجلب من الويب..." complete={stringValue(props.input.url)} part={props.part}>
...
function WebSearch(props: ToolProps) {
  return (
    <InlineTool icon="◈" pending="جارٍ البحث في الويب..." complete={stringValue(props.input.query)} part={props.part}>
  ...
  return (
    <InlineTool
      ...
      onClick={() => {
        ...
        const status = retry()
        if (status) void DialogAlert.show(dialog, "إعادة محاولة الخطأ", status.message)
      }}
    >
      {content()}
    </InlineTool>
...
function Edit(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="←" pending="جارٍ تحضير التحرير..." complete={stringValue(props.input.filePath)} part={props.part}>
...
function ApplyPatch(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="%" pending="جارٍ تحضير الرقعة..." failure="Patch failed" complete={false} part={props.part}>
          Patch
        </InlineTool>
      </Match>
    </Switch>
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool
          icon="⚙"
          pending="جارٍ تحديث المهام..."
          failure="فشل تحديث المهمة"
          complete={false}
          part={props.part}
        >
          Updating todos...
...
function Question(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="→" pending="جارٍ طرح الأسئلة..." complete={count()} part={props.part}>
...
function Skill(props: ToolProps) {
  return (
    <InlineTool icon="→" pending="جارٍ تحميل المهارة..." complete={stringValue(props.input.name)} part={props.part}>
