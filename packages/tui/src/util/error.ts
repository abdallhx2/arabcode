...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    const suggestions = Array.isArray(model.suggestions)
      ...
      : []
    return [
      `النموذج غير موجود: ${field(model, "providerID")}/${field(model, "modelID")}`,
      ...(suggestions.length ? ["هل تقصد: " + suggestions.join(", ")] : []),
      "جرّب الأمر `arabcode models` لعرض النماذج المتاحة",
      "أو تحقق من أسماء المزوّد/النموذج في ملف الإعدادات (opencode.json)",
    ].join("\n")
  }

  const provider = configData(input, "ProviderInitError")
  if (provider)
    return `فشل تهيئة المزوّد "${field(provider, "providerID")}". تحقق من بيانات الاعتماد والإعدادات.`

  const json = configData(input, "ConfigJsonError")
  if (json) {
    const message = field(json, "message")
    return `ملف الإعدادات في ${field(json, "path")} ليس بصيغة JSON(C) صالحة` + (message ? `: ${message}` : "")
  }

  const directory = configData(input, "ConfigDirectoryTypoError")
  if (directory) {
    return `المجلد "${field(directory, "dir")}" في ${field(directory, "path")} غير صالح. أعد تسمية المجلد إلى "${field(directory, "suggestion")}" أو أزله. هذا خطأ إملائي شائع.`
  }
  ...
  if (invalid) {
    ...
    const issues = Array.isArray(invalid.issues)
      ...
      : []
    return [
      `الإعدادات غير صالحة${path && path !== "config" ? ` في ${path}` : ""}` + (message ? `: ${message}` : ""),
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `فشل خادم MCP "${name}". ملاحظة: arabcode لا يدعم مصادقة MCP بعد.`
  }
  return undefined
}
  ...
  if (typeof error === "object" && error !== null) {
    try {
      ...
      if (json === "{}") {
        ...
        return names.length === 0 ? `${prefix} (بلا رسالة)` : `${prefix} { ${names.join(", ")} }`
      }
      return json
    } catch {
      return "خطأ غير متوقع (تعذّر تسلسله)"
    }
  }
