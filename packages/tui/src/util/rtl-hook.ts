...
/**
 * حارس بنيوي: أي `renderer.resume()` (تعليق ctrl+z، محرر خارجي، أو أي مسار
 * مستقبلي) يعيد تهيئة الطرفية فيُسقط وضع BiDi explicit — فتعود VTE لإعادة
 * الترتيب الضمنية وينعكس العربي حتى إعادة التشغيل. الغلاف يعيد بثّ التسلسل
 * بعد كل resume تلقائياً بدل الاعتماد على تذكّر كل موضع استدعاء.
 */
export function installBidiResumeGuard(renderer: unknown): void {
  const r = renderer as { resume?: () => void; __rtlResumeGuard?: boolean }
  if (r.__rtlResumeGuard || typeof r.resume !== "function") return
  r.__rtlResumeGuard = true
  const orig = r.resume.bind(r)
  r.resume = () => {
    orig()
