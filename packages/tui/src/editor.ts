...
import { BIDI_EXPLICIT_ENTER, rtlMode } from "./util/rtl"
import { writeRendererEscape } from "./util/rtl-hook"
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      child.on("exit", (code, signal) => {
        ...
        reject(new Error(`خرج المحرر ${signal ? `بالإشارة ${signal}` : `برمز الخروج ${code}`}`))
      })
    })
    ...
    // المحرر الخارجي قد يعيد ضبط أوضاع الطرفية — إعادة تأكيد وضع BiDi explicit
    if (rtlMode() === "app") writeRendererEscape(input.renderer, BIDI_EXPLICIT_ENTER)
