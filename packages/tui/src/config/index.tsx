...
export const LeaderTimeout = Schema.Int.check(Schema.isGreaterThan(0)).annotate({
  description: "مهلة المفتاح القائد بالمللي ثانية",
})
...
export const ScrollAcceleration = Schema.Struct({
  enabled: Schema.Boolean.annotate({ description: "تفعيل تسريع التمرير" }),
}).annotate({ description: "إعدادات تسريع التمرير" })
...
}).annotate({ description: "إعدادات التنبيه والصوت" })

const PromptSize = Schema.Int.check(Schema.isGreaterThan(0))
export const Prompt = Schema.Struct({
  max_height: Schema.optional(PromptSize).annotate({ description: "أقصى ارتفاع لمربع الكتابة" }),
...
}).annotate({ description: "إعدادات حجم مربع الكتابة" })
...
export const Info = Schema.Struct({
  ...
  scroll_speed: Schema.optional(ScrollSpeed).annotate({ description: "سرعة تمرير الواجهة" }),
  scroll_acceleration: Schema.optional(ScrollAcceleration),
  diff_style: Schema.optional(DiffStyle),
  mouse: Schema.optional(Schema.Boolean).annotate({ description: "تفعيل أو تعطيل التقاط الماوس (الافتراضي: true)" }),
})
