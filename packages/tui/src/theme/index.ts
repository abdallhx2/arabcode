...
import arabcode from "./assets/arabcode.json" with { type: "json" }
...
export function resolveTheme(theme: ThemeJson, mode: "dark" | "light") {
  ...
  function resolveColor(c: ColorValue, chain: string[] = []): RGBA {
    ...
    if (typeof c === "string") {
      ...
      if (chain.includes(c)) {
        throw new Error(`مرجع لون دائري: ${[...chain, c].join(" -> ")}`)
      }

      const next = defs[c] ?? theme.theme[c as ThemeColor]
      if (next === undefined) {
        throw new Error(`مرجع اللون "${c}" غير موجود في defs أو theme`)
      }
