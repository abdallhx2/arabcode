...
import type { ProjectDirectories } from "@arabcode/sdk/v2"
...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current) return [{ title: "جارٍ تحميل مجلدات المشروع...", value: undefined }]
    const roots = [...(data ?? [])]
    ...
    if (roots.length === 0) return [{ title: "لا توجد مجلدات للمشروع", value: undefined }]
    ...
    return list.map((item) => {
      ...
      return {
        ...
        titleView: isRemoving ? (
          <span style={{ fg: theme.error }}>جارٍ حذف {item.location}</span>
        ) : deleting ? (
          <span style={{ fg: theme.text }}>اضغط {deleteHint()} مرة أخرى للتأكيد</span>
        ) : suffix ? (
          <>
        ...
        category: item.root.directory === current ? "الحالي" : "أخرى",
        titleWidth,
    ...
    if (result.error) {
      ...
      if ("data" in result.error && result.error.data.forceRequired) {
        ...
        const choice = await DialogWorkspaceFileChanges.show(dialog, status?.data ?? [], {
          title: "حذف نسخة العمل؟",
          message: "تحتوي نسخة العمل هذه على تغييرات في الملفات. هل تريد حذفها على أي حال؟",
        })
        if (choice !== "yes") {
          reopen()
          return
