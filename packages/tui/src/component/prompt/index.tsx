...
import type { AssistantMessage, FilePart, UserMessage } from "@arabcode/sdk/v2"
...
import { applyProseFix, scanProse, type ProseCandidate } from "../../util/layout-fix"
...
export function Prompt(props: PromptProps) {
  ...
  const [layoutHintSource, setLayoutHintSource] = createSignal("")
  ...
  function promptModelWarning() {
    toast.show({
      variant: "warning",
      message: "اربط مزوّداً لإرسال الطلبات",
      duration: 3000,
    })
  ...
  const promptCommands = createMemo(() =>
    [
      {
        title: "مسح مربع الكتابة",
        name: "prompt.clear",
        category: "مربع الكتابة",
        hidden: true,
        run: () => {
          clearPrompt()
          dialog.clear()
  ...
  useBindings(() => {
    return {
      ...
      bindings: [{ key: "escape", desc: "الخروج من وضع الطرفية", group: "مربع الكتابة", cmd: () => setStore("mode", "normal") }],
    }
  })
  ...
  useBindings(() => {
    return {
      ...
      bindings: [{ key: "backspace", desc: "الخروج من وضع الطرفية", group: "مربع الكتابة", cmd: () => setStore("mode", "normal") }],
    }
  })
  ...
  // arabcode: التلميح النشط يستهلك Tab (قبول) وEscape (رفض لهذا السطر) — طبقة
  // مقيّدة بمربع الكتابة (target) وشرط تفعيل layoutHint، على نمط طبقات prompt.history.* أعلاه،
  // كي تسبق ربط Tab العام (agent_cycle) عند تركيز مربع الكتابة.
  useBindings(() => {
    return {
      target: inputTarget,
      enabled: inputTarget() !== undefined && !props.disabled && layoutHint().length > 0,
      bindings: [
        {
          key: "tab",
          desc: "قبول تصحيح التخطيط",
          group: "مربع الكتابة",
          cmd: () => {
            const current = input.plainText
            // arabcode: لا تطبيق على تلميح قديم أو ومؤشر ليس بنهاية السطر — امسح التلميح بدل التخريب
            if (current !== layoutHintSource() || input.cursorOffset !== Bun.stringWidth(current)) {
              setLayoutHint([])
              return
            }
            const fixed = applyProseFix(current, layoutHint())
  ...
  async function submitInner() {
    ...
    if (sessionID == null) {
      ...
      if (res.error) {
        ...
        toast.show({
          message: "فشل إنشاء الجلسة. افتح وحدة التحكم لمزيد من التفاصيل.",
          variant: "error",
        })
  ...
  // في السكون يميل الشريط اللونيّ للـ«يشمي» (هوية عرب كود)، ثم ينزلق نحو لون الوكيل
  // النشط عند ظهوره — فالحافة القائدة للكتابة تبقى معبّرة عن الهوية حتى قبل بدء الوكيل.
  const barBase = createMemo(() => tint(theme.border, theme.primary, 0.4))
  const borderHighlight = createMemo(() => tint(barBase(), highlight(), agentMetaAlpha()))
  ...
  const placeholderText = createMemo(() => {
    ...
    if (store.mode === "shell") {
      ...
      return `شغّل أمراً... "${example}"`
    }
    if (!list().length) return undefined
    return `اكتب أي شيء... "${list()[store.placeholder % list().length]}"`
  })
  ...
  return (
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
            ...
            <textarea
              ...
              onContentChange={() => {
                ...
                if (!layoutHintDismissed() && !value.startsWith("/") && !auto()?.visible && value.endsWith(" ")) {
                  ...
                  setLayoutHintSource(value)
                } else if (layoutHint().length > 0 && (auto()?.visible || value.length === 0 || value !== layoutHintSource())) {
                  setLayoutHint([])
                }
                syncExtmarksWithPromptParts()
              ...
              syntaxStyle={syntax()}
            />
            <box flexDirection="row-reverse" flexShrink={0} paddingTop={1} gap={1} justifyContent="space-between">
                  ...
                  {(agent) => (
                    <>
                      ...
                      <Show when={store.mode === "normal"}>
                        <box flexDirection="row" gap={1}>
                          <text fg={fadeColor(theme.accent, modelMetaAlpha() * 0.7)}>{KNOT}</text>
                          <text
                            flexShrink={0}
        ...
        <box width="100%" flexDirection="row-reverse" justifyContent="space-between">
          <Switch>
            <Match when={status().type !== "idle"}>
              ...
              >
                <box flexShrink={0} flexDirection="row-reverse" gap={1}>
                  <box marginRight={1}>
                  ...
                  <box flexDirection="row" gap={1} flexShrink={0}>
                    {(() => {
                      ...
                      const handleMessageClick = () => {
                        ...
                        if (!r) return
                        if (isTruncated()) {
                          void DialogAlert.show(dialog, "إعادة محاولة الخطأ", r.message)
                        }
                      }
                ...
                <text fg={store.interrupt > 0 ? theme.primary : theme.text}>
                  ...
                  <span style={{ fg: store.interrupt > 0 ? theme.primary : theme.textMuted }}>
                    {store.interrupt > 0 ? "اضغط ثانيةً للمقاطعة" : "مقاطعة"}
                  </span>
                </text>
              </box>
            </Match>
              ...
              <Switch>
                <Match when={store.mode === "normal"}>
                  <Switch>
                    ...
                    <Match when={true}>
                      <text fg={theme.text}>
                        {agentShortcut()} <span style={{ fg: theme.textMuted }}>الوكلاء</span>
                      </text>
                    </Match>
                  </Switch>
                  <text fg={theme.text}>
                    {paletteShortcut()} <span style={{ fg: theme.textMuted }}>الأوامر</span>
                  </text>
                </Match>
                <Match when={store.mode === "shell"}>
                  <text fg={theme.text}>
                    esc <span style={{ fg: theme.textMuted }}>اخرج من وضع الطرفية</span>
                  </text>
                </Match>
              </Switch>
            </box>
