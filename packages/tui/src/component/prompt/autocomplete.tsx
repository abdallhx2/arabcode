...
import { fixSlashCommand } from "../../util/layout-fix"
import type { FileSystemEntry } from "@arabcode/sdk/v2"
...
export function Autocomplete(props: {
  ...
  function maybeFixSlashLayout(value: string): boolean {
    // arabcode: صحّح فقط والمؤشر في نهاية السطر — تعديل منتصف الكلمة يترك ذيلاً بعد المؤشر
    if (props.input().cursorOffset !== Bun.stringWidth(value)) return false
        ...
        <Index
          ...
          fallback={
            <box paddingRight={1} paddingLeft={1}>
              <text fg={theme.textMuted}>لا توجد عناصر مطابقة</text>
            </box>
          }
        >
          {(option, index) => (
