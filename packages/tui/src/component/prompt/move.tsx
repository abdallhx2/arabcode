...
export function usePromptMove(input: { projectID: () => string | undefined; sessionID: () => string | undefined }) {
  ...
  async function create(context?: string) {
    ...
    try {
      ...
      if (!directory) throw new Error("لم يتم إرجاع مجلد نسخة المشروع")

      // Call a location-based route to make sure it's bootstrapped
      // before moving on
      await sdk.client.path.get({ directory }, { throwOnError: true })

      setProgress("جارٍ إنشاء الجلسة")
      return directory
    } catch (err) {
      homeDestination?.clear()
      setProgress(undefined)
      setCreating(false)
      toast.show({ title: "فشل إنشاء مساحة العمل", message: errorMessage(err), variant: "error" })
      return
    }
  }
  ...
  function startSubmit() {
    if (progress()) setProgress("جارٍ إرسال الرسالة")
  }
