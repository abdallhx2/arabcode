import type { ExperimentalWorkspaceAdapterListResponse, Workspace } from "@arabcode/sdk/v2"
...
async function loadWorkspaceAdapters(input: {
  ...
  try {
    ...
    input.toast.show({
      title: "فشل تحميل محوّلات مساحة العمل",
  ...
  if (!result?.data) {
    if (result?.error && "name" in result.error && result.error.name === "VcsApplyError") {
      await DialogAlert.show(
        input.dialog,
        "تعذّر نقل الجلسة",
        "تعذّر تطبيق تغييرات الملفات على مساحة العمل هذه. توجد فيها تغييرات متعارضة أو أنها مبنية على فرع (branch) مختلف. لم يتم نقل الجلسة.",
      )
      return false
    }
    ...
    input.toast.show({
      title: "فشل نقل الجلسة",
      message: errorMessage(result?.error ?? "لا استجابة"),
      variant: "error",
    })
    return false
  }
  ...
  const options = createMemo<DialogSelectOption<WorkspaceSelectValue>[]>(() => {
    ...
    return [
      ...list.map((adapter) => ({
        ...
        category: "مساحة عمل جديدة",
      })),
      {
        title: "بلا",
        value: { type: "none" as const },
        description: "استخدام المشروع المحلي",
        category: "اختيار مساحة العمل",
      },
      ...
      ...(hasMore
        ? [
            {
              title: "عرض كل مساحات العمل",
              value: { type: "existing-list" as const },
              description: "الاختيار من بين كل مساحات العمل",
              category: "اختيار مساحة العمل",
            },
          ]
        : []),
