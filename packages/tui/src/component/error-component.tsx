...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  const message = props.error.message || "حدث خطأ غير معروف."
  const stack = props.error.stack || "لا يتوفر تتبع للمكدس."
  ...
  const actions = [
    { key: "c", label: () => (copied() ? "✓ تم النسخ" : "نسخ التقرير"), copy: true, onUse: copyReport },
    { key: "r", label: () => "إعادة التشغيل", onUse: props.reset },
    { key: "q", label: () => "خروج", onUse: () => exit() },
  ]
      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <box flexDirection="column" alignItems="center" flexShrink={0}>
          ...
          <Show when={showSubtext()}>
            <text fg={colors.muted}>أوقف خطأ غير متوقع الجلسة.</text>
          </Show>
        </box>
        ...
        {/* Actions */}
        <box flexDirection="row-reverse" flexWrap="wrap" justifyContent="center" gap={2} rowGap={1} flexShrink={0}>
          <For each={actions}>
            {(action, index) => {
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "تم نسخ التقرير — الصقه في مشكلة GitHub جديدة."
                : "انسخ التقرير وافتح مشكلة على GitHub لمساعدتنا في إصلاح ذلك."}
            </text>
            <text fg={colors.muted}>arabcode {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
