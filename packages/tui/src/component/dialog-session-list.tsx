...
export function DialogSessionList() {
  ...
  const quickSwitchFooterHints = createMemo(() => {
    ...
    return hint && local.session.slots().length > 0 ? [{ title: "تبديل", label: hint }] : []
  })
  ...
  const options = createMemo(() => {
    ...
    function buildOption(id: string, category: string) {
      ...
      const gutter = isWorking
        ...
        : slot !== undefined
          ...
          : undefined
      return {
        title: isDeleting ? `اضغط ${deleteHint()} مرة أخرى للتأكيد` : x.title,
    ...
    const remaining = displayOrder
      ...
      .map((id) => {
        ...
        return buildOption(id, label === today ? "اليوم" : label)
      })
      .filter((x) => x !== undefined)

    return [...pinned.map((id) => buildOption(id, "المثبّتة")).filter((x) => x !== undefined), ...remaining]
  })
