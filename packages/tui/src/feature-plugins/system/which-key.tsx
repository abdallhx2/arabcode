...
import type { TuiPlugin, TuiPluginApi } from "@arabcode/plugin/tui"
    ...
    commands: [
      {
        ...
        title: "مجموعة الاختصارات السابقة",
        desc: "عرض مجموعة الاختصارات السابقة",
        category: "النظام",
        run() {
          moveGroup(-1)
        },
      },
      {
        ...
        title: "مجموعة الاختصارات التالية",
        desc: "عرض مجموعة الاختصارات التالية",
        category: "النظام",
        run() {
          moveGroup(1)
        },
      },
      {
        ...
        title: "تمرير الاختصارات لأعلى",
        desc: "تمرير لوحة الاختصارات لأعلى",
        category: "النظام",
        run() {
          scroll(-columns())
        },
      },
      {
        ...
        title: "تمرير الاختصارات لأسفل",
        desc: "تمرير لوحة الاختصارات لأسفل",
        category: "النظام",
        run() {
          scroll(columns())
        },
      ...
      {
        name: command.pageUp,
        title: "صفحة اختصارات لأعلى",
        desc: "صفحة لوحة الاختصارات لأعلى",
        category: "النظام",
        run() {
          scroll(-pageSize())
        },
      },
      {
        ...
        title: "صفحة اختصارات لأسفل",
        desc: "صفحة لوحة الاختصارات لأسفل",
        category: "النظام",
        run() {
          scroll(pageSize())
        },
      ...
      {
        name: command.home,
        title: "الاختصار الأول",
        desc: "الانتقال إلى أول اختصار",
        category: "النظام",
        run() {
          setOffset(0)
        },
      ...
      {
        name: command.end,
        title: "الاختصار الأخير",
        desc: "الانتقال إلى آخر اختصار",
        category: "النظام",
        run() {
          setOffset(maxOffset())
        },
    ...
    <Show when={visible()}>
      ...
      >
        <Show when={headerVisible()}>
          <box width="100%" flexDirection="row-reverse" justifyContent="center" gap={tabGap()} flexShrink={0}>
        ...
        <box height={rows()} flexShrink={0} flexDirection="column">
          <Show when={shown().length > 0} fallback={<text fg={look().muted}>لا توجد اختصارات متاحة</text>}>
            <For each={rowIndexes()}>
              {(row) => (
                <box width="100%" flexDirection="row-reverse" justifyContent="center" gap={COLUMN_GAP}>
                  <For each={shown()}>
                    {(column) => {
                      ...
                      return (
                        <box width={columnWidth()} flexDirection="row-reverse" gap={1} justifyContent="space-between">
                          <Show when={item()}>
                            {(value) => (
                              <Show
                                when={entry()}
        ...
        <Show when={footerVisible()}>
          ...
          <box width="100%" flexDirection="row-reverse" justifyContent="space-between" flexShrink={0}>
            <box>
  ...
  api.keymap.registerLayer({
    ...
    commands: [
      ...
      },
      {
        ...
        title: "تبديل معاينة المفتاح المعلّق",
        desc: "عرض لوحة الاختصارات تلقائياً لتسلسلات المفاتيح المعلّقة في وضع التراكب",
        category: "النظام",
        run() {
