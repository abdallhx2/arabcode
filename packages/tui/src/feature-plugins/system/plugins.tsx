import type { TuiPlugin, TuiPluginApi, TuiPluginStatus } from "@arabcode/plugin/tui"
...
function Install(props: { api: TuiPluginApi }) {
  ...
  useBindings(() => ({
    enabled: !busy(),
    bindings: [{ key: "tab", desc: "تبديل نطاق التثبيت", group: "الإضافات", cmd: () => setGlobal((value) => !value) }],
  }))

  return (
    <props.api.ui.DialogPrompt
      title="تثبيت إضافة"
      placeholder="اسم حزمة npm"
      busy={busy()}
      busyText="جارٍ تثبيت الإضافة..."
      description={() => (
      ...
      onConfirm={(raw) => {
        ...
        if (!mod) {
          props.api.ui.toast({
            variant: "error",
            message: "اسم حزمة الإضافة مطلوب",
          })
          return
        }
          ...
          .then((out) => {
            if (!out.ok) {
              ...
              if (out.missing) {
                props.api.ui.toast({
                  variant: "info",
                  message: "تحقق من إعدادات سجل npm/المصادقة وحاول مجدداً.",
                })
              }
              show(props.api)
              return
            ...
            if (!out.tui) {
              props.api.ui.toast({
                variant: "info",
                message: "الحزمة لا تحتوي على هدف TUI لتحميله في هذا التطبيق.",
              })
              show(props.api)
              return
            }
