import type { Event } from "@arabcode/sdk/v2"
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@arabcode/plugin/tui"
...
function sessionErrorMessage(error: SessionError) {
  ...
  if (data && typeof data === "object" && "message" in data && data.message === "SSE read timed out") {
    return "توقّف النموذج عن الاستجابة"
  }
...
const tui: TuiPlugin = async (api) => {
  ...
  api.event.on("question.asked", (event) => {
    ...
    notify(api, event.properties.sessionID, "السؤال يحتاج إدخالاً", "question")
  })
  ...
  api.event.on("permission.asked", (event) => {
    ...
    notify(api, event.properties.sessionID, "الإذن يحتاج إدخالاً", "permission")
  })
