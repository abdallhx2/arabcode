    ...
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box gap={1}>
        <box alignSelf="flex-end">
          <text fg={theme.text}>اسم الملف:</text>
        </box>
        <textarea
          width="100%"
          onSubmit={() => {
      ...
      <box flexDirection="column" alignItems="flex-end">
        ...
        >
          ...
          <text fg={store.active === "thinking" ? theme.primary : theme.text}>تضمين التفكير</text>
        </box>
        <box
          flexDirection="row-reverse"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "toolDetails" ? theme.primary : theme.text}>تضمين تفاصيل الأدوات</text>
        </box>
        <box
          flexDirection="row-reverse"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "assistantMetadata" ? theme.primary : theme.text}>تضمين البيانات الوصفية للمساعد</text>
        </box>
        <box
          flexDirection="row-reverse"
          gap={2}
        ...
        >
          ...
          <text fg={store.active === "openWithoutSaving" ? theme.primary : theme.text}>فتح دون حفظ</text>
        </box>
      </box>
      <Show when={store.active !== "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          اضغط <span style={{ fg: theme.text }}>space</span> للتبديل، و<span style={{ fg: theme.text }}>return</span>{" "}
          للتأكيد
        </text>
      </Show>
      <Show when={store.active === "filename"}>
        <text fg={theme.textMuted} paddingBottom={1}>
          اضغط <span style={{ fg: theme.text }}>return</span> للتأكيد، و<span style={{ fg: theme.text }}>tab</span>{" "}
          للخيارات
        </text>
      </Show>
    </box>
