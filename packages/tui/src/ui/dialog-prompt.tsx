    ...
    <box paddingLeft={2} paddingRight={2} gap={1}>
      ...
      <box gap={1}>
        ...
        <textarea
          ...
          placeholder={props.placeholder ?? "أدخل نصاً"}
        ...
        <Show when={props.busy}>
          <Spinner color={theme.textMuted}>{props.busyText ?? "جارٍ العمل..."}</Spinner>
        </Show>
      </box>
      <box paddingBottom={1} gap={1} flexDirection="row-reverse">
        <Show when={!props.busy} fallback={<text fg={theme.textMuted}>جارٍ المعالجة...</text>}>
          <Show when={submitShortcut()}>
            <text fg={theme.text}>
              {submitShortcut()} <span style={{ fg: theme.textMuted }}>إرسال</span>
            </text>
          </Show>
        </Show>
      </box>
