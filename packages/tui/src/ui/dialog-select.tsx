...
import { KNOT } from "../component/ornament"
...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  const filtered = createMemo(() => {
    ...
    // تطبيع عربي للبحث: يطابق رغم اختلاف الهمزة/الألف/التاء المربوطة والحركات.
    const needle = searchKey(store.filter)
    const options = pipe(
      props.options,
    ...
    const result = fuzzysort
      .go(needle, options, {
        keys: [(o) => searchKey(o.title), (o) => searchKey(o.category ?? "")],
      ...
      bindings: [
        ...
        ...(visible.length
          ? [
              {
                key: "tab",
                desc: "إجراء النافذة التالي",
                group: "الحوار",
                cmd: () => moveAction(1),
              },
              {
                key: "shift+tab",
                desc: "إجراء النافذة السابق",
                group: "الحوار",
                cmd: () => moveAction(-1),
              },
            ]
  ...
  return (
    <box gap={1} paddingBottom={1} flexGrow={1}>
          ...
          fallback={
            props.emptyView ?? (
              <box paddingLeft={4} paddingRight={4} paddingTop={1}>
                <text fg={theme.textMuted}>لا توجد نتائج</text>
              </box>
            )
          }
        >
          ...
          >
            <For each={grouped()}>
              {([category, options], index) => (
                <>
                  <Show when={category}>
                    <box paddingTop={index() > 0 ? 1 : 0} paddingRight={3} alignItems="flex-end">
                      <Show
                      ...
                      return (
                        ...
                        >
                          <box
                            ...
                            paddingRight={current() || option.gutter ? 1 : 3}
                            paddingLeft={3}
                            gap={1}
                            backgroundColor={
                              active()
                          ...
                          >
                            <Show when={!current() && option.margin}>
                              <box position="absolute" right={1} flexShrink={0}>
                                {option.margin}
                              </box>
                            </Show>
                            <Option
                          ...
                          <For each={option.details}>
                            {(detail) => (
                              <box paddingLeft={3} paddingRight={3} alignItems="flex-end">
      ...
      <Show when={props.footer || visibleActions().length} fallback={<box flexShrink={0} />}>
        <box paddingRight={4} paddingLeft={2} flexDirection="row-reverse" justifyContent="space-between" flexShrink={0}>
...
function Option(props: {
  ...
  return (
    <>
      ...
      <box flexGrow={1} alignItems="flex-end" paddingRight={3} overflow="hidden">
        <text
          fg={text()}
