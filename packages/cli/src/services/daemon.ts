...
import { createOpencodeClient } from "@arabcode/sdk/v2/client"
import { ServerAuth } from "@arabcode/server/auth"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const healthy = Effect.fnUntraced(function* () {
      ...
      return yield* Effect.fail(new Error("الخادم المسجّل غير سليم"))
    })

    const compatible = Effect.fnUntraced(function* () {
      const info = yield* healthy()
      if (info.version === InstallationVersion) return info
      return yield* Effect.fail(new Error("إصدار الخادم المسجّل لا يطابق العميل"))
    })
    ...
    const awaitStopped = Effect.fnUntraced(function* (pid: number) {
      ...
      return yield* Effect.fail(new Error(`عملية الخادم ${pid} لا تزال قيد التشغيل`))
    })
    ...
    const start = Effect.fn("cli.daemon.start")(function* () {
      ...
      if (!compiled && entrypoint === undefined)
        return yield* Effect.fail(new Error("فشل تحديد نقطة دخول CLI"))
      yield* Effect.try({
        try: () => {
        ...
        catch: (cause) => new Error("فشل تشغيل الخادم", { cause }),
      })

      return yield* compatible().pipe(
        Effect.retry(Schedule.spaced("50 millis").pipe(Schedule.both(Schedule.recurs(100)))),
        Effect.map((info) => info.url),
        Effect.mapError(() => new Error("فشل تشغيل الخادم")),
      )
    })
